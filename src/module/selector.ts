 ...
 */
import { filter as _filter } from "./rule/filter";
import { replace as _replace } from "./rule/replace";
...
interface Options {
  ...
  to: string;
  sex: number;
  optimal?: boolean; // 可选属性
}
// 获得最简
const getOptimal = function (options: Options) {
  ...
  for (let i = 0; i < from_chain.length && i < to_chain.length; i++) {
    if (from_chain[i] == to_chain[i]) {
      ...
      sex = from_chain[i].match(/^([fhs1](&[ol\d]+)?|[olx]b)(&[ol\d]+)?/)
        ? 1
        : 0;
      continue;
    } else {
      if (
        getGenById(from_chain[i]) == getGenById(to_chain[i]) &&
        from_chain[i].match(/^[xol][bs]|^[sd]/) &&
        to_chain[i].match(/^[xol][bs]|^[sd]/)
      ) {
        let form_type = from_chain[i]
          .replace(/&([ol\d]+)/, "")
          .replace(/^[xol]([bs])/, "$1");
        let to_type = to_chain[i]
          .replace(/&([ol\d]+)/, "")
          .replace(/^[xol]([bs])/, "$1");
        ...
        let from_attr =
          from_chain[i].match(/&([ol\d]+)/)?.[1] ||
          from_chain[i].match(/([ol])[bs]/)?.[1] ||
          "";
        let to_attr =
          to_chain[i].match(/&([ol\d]+)/)?.[1] ||
          to_chain[i].match(/([ol])[bs]/)?.[1] ||
          "";
        if (from_attr && to_attr) {
          if (!isNaN(Number(from_attr)) && !isNaN(Number(to_attr))) {
            if (+from_attr > +to_attr) {
              from_chain[i] = from_chain[i]
                .replace(/^[xol]b|^s/, "lb")
                .replace(/^[xol]s|^d/, "ls");
            ...
            (!isNaN(Number(from_attr)) && to_attr == "o") ||
            (from_attr == "l" && !isNaN(Number(to_attr)))
          ) {
            from_chain[i] = from_chain[i]
              .replace(/^[xol]b|^s/, "lb")
              .replace(/^[xol]s|^d/, "ls");
          } else if (
            (!isNaN(Number(from_attr)) && to_attr == "l") ||
            (from_attr == "o" && !isNaN(Number(to_attr)))
          ) {
            from_chain[i] = from_chain[i]
              .replace(/^[xol]b|^s/, "ob")
              .replace(/^[xol]s|^d/, "os");
          ...
          to = to_chain.slice(i + 1).join(",");
          sex = to_chain[i].match(/^([fhs1](&[ol\d]+)?|[olx]b)(&[ol\d]+)?/)
            ? 1
            : 0;
...
// 中文获取选择器
export function getSelectors(str: string) {
  str = str
    .replace(/之/g, "的")
    .replace(/吾之?(.+)/, "$1")
    .replace(/我的?(.+)/, "$1");
  ...
  while (lists.length) {
    ...
    const keywords = [name];
    let getList = function (name: string) {
      // 词义扩展
...
// 合并选择器，查找两个对象之间的关系
export function mergeSelector(param: Options) {
  let { from: from_selector, to: to_selector, sex: my_sex } = param;
  if (my_sex < 0) {
    const fromSex = from_selector.match(/^,[w1]/)
      ? 1
      : from_selector.match(/^,[h0]/)
      ? 0
      : -1;
    const toSex = to_selector.match(/^,[w1]/)
      ? 1
      : to_selector.match(/^,[h0]/)
      ? 0
      : -1;
  ...
  const result: { selector: string; sex: number }[] = [];
...
export function expandSelector(selector: string): string[] {
  ...
  const getSelector = function (selector: string) {
    if (!hash[selector]) {
      hash[selector] = true;
      let s = "";
      do {
      ...
      if (selector.match(/,[mwd0](&[ol\d+])?,w|,[hfs1](&[ol\d]+)?,h/)) {
        //同志关系去除
        return false;
      }
...
// 选择器转ID
export function selector2id(selector: string, sex?: number): string[] {
  // 处理 sex 未传入的情况
  if (sex === undefined) {
    sex = -1;
  }
  ...
  const result = expandSelector(selector).map((selector) =>
    selector.replace(/,[01]/, "").substr(1)
  ); //去前面逗号和性别信息
